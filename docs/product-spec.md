...
When every unreadable variant is still bound to the same unconfirmed external
directory observed by the current Snapshot, this is a source-trust decision,
not drift: `variant_finding.state` is `source_confirmation_required` and its
read-only action opens the current escaping-link Finding. Exact-load actions
remain absent until the source is confirmed. A missing or retargeted link still
requires a new Scan.
