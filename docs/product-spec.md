...
`plan --stdin --json` returns a bounded decision-complete summary: total change counts, operation groups, affected-scope counts with at most ten Skill IDs, before/after impact, risk, reversibility, and the immutable Plan ID. A Finding-derived Roster Plan also persists `selection_evidence`, including forced, positive-signal, and stable-fallback Core counts plus at most five named Core selections per Agent with their selection reasons. When fallback selection dominates any affected Agent, the Plan carries typed `uncertainty` with `review_required: true`; absence of usage evidence remains explicitly non-negative. It does not inline filesystem operations, complete Core selections, or complete large ID collections. `plan --show PLAN_ID --json` is the explicit full-detail path; it reads the stored immutable Plan, returns every named Core selection and reason, and never mutates files. Plans created before complete Core selections were persisted remain readable with their original aggregate evidence.
