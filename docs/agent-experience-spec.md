...
5. **Safety boundary:** read-only so far, canonical deletion count, reversibility, and uncertainty. For semantic Roster Plans, include forced, positive-signal, and stable-fallback Core counts plus the bounded named Core preview and its reasons; fallback-dominated selection remains a review-required proposal.
