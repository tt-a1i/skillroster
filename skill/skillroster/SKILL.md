...
1. Run `skillroster scan --json`, then selector-free `skillroster report --json`; this is the bounded Summary view, and `--summary` is its explicit alias. Use the compact result for the first user-facing diagnosis. Read `finding_rollups` to state the complete scale of each Finding group without loading the exhaustive report; use the three selected `findings` only for the highest-priority decisions. Reserve top-level `report --full --json` for a deliberate exhaustive diagnostic export outside the normal Agent context.
